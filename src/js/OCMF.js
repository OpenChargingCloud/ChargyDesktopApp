...
class OCMF {
    ...
    async tryToParseOCMFv0_1(OCMFData, PublicKey) {
        // {
        //     "FV": "0.1",
        //     "VI": "ABL",
        //     "VV": "1.4p3",
        ...
        // }
        try {
            let VendorInformation = OCMFData.VI != null ? OCMFData.VI.trim() : ""; // Some text about the manufacturer, model, variant, ... of e.g. the vendor.
            let VendorVersion = OCMFData.VV != null ? OCMFData.VV.trim() : ""; // Software version of the vendor.
            let paging = OCMFData.PG != null ? OCMFData.PG.trim() : ""; // Paging, as this data might be part of a larger context.
            let transactionType = 0 /* undefined */;
            switch (paging[0].toLowerCase()) {
                case 't':
                    transactionType = 2 /* transaction */;
                    break;
                case 'f':
                    transactionType = 1 /* fiscal */;
                    break;
            }
            let pagingId = paging.substring(1);
            let MeterVendor = OCMFData.MV != null ? OCMFData.MV.trim() : ""; // Vendor of the device, optional.
            let MeterModel = OCMFData.MM != null ? OCMFData.MM.trim() : ""; // Model of the device, optional.
            let MeterSerial = OCMFData.MS != null ? OCMFData.MS.trim() : ""; // Serialnumber of the device, might be optional.
            let MeterFirmware = OCMFData.MF != null ? OCMFData.MF.trim() : ""; // Software version of the device.
            return {
    ...
    async tryToParseOCMF(OCMFValues, PublicKey) {
        try {
            var CTR = {
            ...
            for (let OCMFData of OCMFValues) {
                let GatewayInformation = OCMFData.GI != null ? OCMFData.GI.trim() : ""; // Some text about the manufacturer, model, variant, ... of e.g. the gateway.
                let GatewaySerial = OCMFData.GS != null ? OCMFData.GS.trim() : ""; // Serial number of the gateway, might be mandatory.
                let GatewayVersion = OCMFData.GV != null ? OCMFData.GV.trim() : ""; // Software version of the gateway.
                let paging = OCMFData.PG != null ? OCMFData.PG.trim() : ""; // Paging, as this data might be part of a larger context.
                let TransactionType = 0 /* undefined */;
                switch (paging[0].toLowerCase()) {
                    case 't':
                ...
                let MeterSerial = OCMFData.MS != null ? OCMFData.MS.trim() : ""; // Serialnumber of the device, might be optional.
                let MeterFirmware = OCMFData.MF != null ? OCMFData.MF.trim() : ""; // Software version of the device.
                let IdentificationStatus = OCMFData.IS != null ? OCMFData.IS : false; // true, if user is assigned, else false.
                let IdentificationLevel = OCMFData.IL != null ? OCMFData.IL.trim() : ""; // optional
                let IdentificationFlags = OCMFData.IF != null ? OCMFData.IF : []; // optional
                let IdentificationType = OCMFData.IT != null ? OCMFData.IT.trim() : ""; // The type of the authentication data.
                let IdentificationData = OCMFData.ID != null ? OCMFData.ID.trim() : ""; // The authentication data.
                let ChargePointIdType = OCMFData.CT != null ? OCMFData.CT.trim() : ""; // Type of the following ChargePointId: EVSEId|ChargingStationId|...
                let ChargePointId = OCMFData.CI != null ? OCMFData.CI.trim() : ""; // The identification of the charge point
    ...
    //#endregion
    //#region Try
    async tryToParseOCMF2(OCMFValues, PublicKey) {
        let commonVersion = "";
        let OCMFDataList = [];
        ...
        for (let OCMFValue of OCMFValues) {
            ...
            if (OCMFSections.length == 3) {
                ...
                else if (OCMFVersion != commonVersion)
        ...
        if (OCMFDataList.length >= 2) {
            switch (commonVersion) {
                ...
                case "1.0":
                    return await this.tryToParseOCMF(OCMFDataList, PublicKey);
                default:
                    return {
                        status: SessionVerificationResult.InvalidSessionFormat,
