...
class Alfen01 {
    ...
    async tryToParseALFENFormat(Content) {
        ...
        try {
            ...
            for (let i = 0; i < signedValues.length; i++) {
                ...
                let PublicKey = base32Decode(elements[3], 'RFC4648'); // 25 bytes; base32 encoded
                let DataSet = base32Decode(elements[4], 'RFC4648'); // 82 bytes; base32 encoded
                let Signature = base32Decode(elements[5], 'RFC4648'); // 48 bytes; base32 encoded; secp192r1
                ...
                let AdapterFWVersion = String.fromCharCode.apply(null, new Uint8Array(DataSet.slice(10, 14))); // ASCII: 76 30 31 34 (v014)
                ...
                let UID = String.fromCharCode.apply(null, new Uint8Array(DataSet.slice(54, 74))). // ASCII: 30 35 38 39 38 41 42 42 00 00 00 00 00 00 00 00 00 00 00 00 => UID: 05 89 8A BB
                    replace(/\0.*$/g, '');
...
class AlfenCrypt01 extends ACrypt {
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (this.curve.keyFromPublic(buf2hex(base32Decode(cryptoResult.publicKey.toUpperCase(), 'RFC4648')), 'hex').
                                    verify(cryptoResult.sha256value.toUpperCase(), cryptoResult.signature)) {
    ...
    async ViewMeasurement(measurementValue, introDiv, infoDiv, PlainTextDiv, HashedPlainTextDiv, PublicKeyDiv, SignatureExpectedDiv, SignatureCheckDiv) {
        ...
        if (PlainTextDiv != null) {
            ...
            this.CreateLine("Meter status", hex2bin(measurementValue.statusMeter, true) + " (" + measurementValue.statusMeter + " hex)<br /><span class=\"statusInfos\">" +
                this.DecodeMeterStatus(measurementValue.statusMeter).join("<br />") + "</span>", result.statusMeter || "", infoDiv, PlainTextDiv);
            this.CreateLine("Adapter status", hex2bin(measurementValue.statusAdapter, true) + " (" + measurementValue.statusAdapter + " hex)<br /><span class=\"statusInfos\">" +
                this.DecodeAdapterStatus(measurementValue.statusAdapter).join("<br />") + "</span>", result.statusAdapter || "", infoDiv, PlainTextDiv);
        ...
        if (PublicKeyDiv != null &&
            ...
            if (!IsNullOrEmpty(result.publicKey))
                PublicKeyDiv.innerHTML = (_b = (_a = result.publicKey.match(/.{1,4}/g)) === null || _a === void 0 ? void 0 : _a.join(" ")) !== null && _b !== void 0 ? _b : "";
        ...
        if (SignatureExpectedDiv != null && result.signature != null) {
            ...
            if (result.signature.r && result.signature.s)
                SignatureExpectedDiv.innerHTML = "r: " + ((_c = result.signature.r.toLowerCase().match(/.{1,8}/g)) === null || _c === void 0 ? void 0 : _c.join(" ")) + "<br />" +
                    "s: " + ((_d = result.signature.s.toLowerCase().match(/.{1,8}/g)) === null || _d === void 0 ? void 0 : _d.join(" "));
            else if (result.signature.value)
                SignatureExpectedDiv.innerHTML = (_f = (_e = result.signature.value.toLowerCase().match(/.{1,8}/g)) === null || _e === void 0 ? void 0 : _e.join(" ")) !== null && _f !== void 0 ? _f : "-";
        }
        //#endregion
    ...
    DecodeMeterStatus(statusValueHex) {
        let statusArray = [];
        try {
    ...
    DecodeAdapterStatus(statusValueHex) {
        let statusArray = [];
        try {
            let status = parseInt(statusValueHex, 16);
...
//# sourceMappingURL=Alfen01.js.map